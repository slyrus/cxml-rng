...
(eval-when (:compile-toplevel :load-toplevel :execute)
  ...
  (defun make-or (v)
    `(or ,@(loop
	      for (min max) across v
	      collect `(range ,min ,max)))))

(clex:deflexer test
    ((digit #.(make-or *digit-ranges*))
     (base #.(make-or *base-char-ranges*))
     (combining #.(make-or *combining-char-ranges*))
     (extender #.(make-or *extender-ranges*))
     (ideographic #.(make-or *ideographic-ranges*))
     ...
     (letter (or base ideographic))
     (name-start-char (or letter #\_ #\:))
     (name-char (or letter digit #\. #\- #\_ #\: combining extender))
  ...
  ((and #\\ (and name-start-char (* name-char)))
